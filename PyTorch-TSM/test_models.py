...
# CMD：
    # TEST：
        # python test_models.py dribbling --weights=pretrained/TSM_somethingv2_RGB_resnet50_shift8_blockres_avg_segment8_e45.pth --test_segments=8 --batch_size=32 -j 24 --test_crops=1 --test_list=dataset/dribbling/test_videofolder.txt

video_prob = []
cate = ['Behind Dribbling', 'Pound Dribbling', 'Cross Over Dribbling', 'One Side Leg Dribbling']
import cv2


def output_video(path, name, p, classes):
    cap = cv2.VideoCapture(path) # 读取视频

    # Define the codec and create VideoWriter object
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')

    outputvideo_name = name + "_Output.mp4"

    out = cv2.VideoWriter(outputvideo_name, fourcc, 5.0, (1280,720)) # 输出视频参数设置
    ...
    while(cap.isOpened()):
        ...
        if ret == True:
            ...
            cv2.rectangle(frame, left_up, right_down, color, thickness)

            # 位置
            word_x = 5
            word_y = 135
            cv2.putText(frame, 'Behind_value : %s' %p[0], (word_x, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1)
            cv2.putText(frame, 'Pound_value : %s' %p[1], (word_x, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1)
            cv2.putText(frame, 'CrossOver_value : %s' %p[2], (word_x, 75), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1)
            cv2.putText(frame, 'OneSideLeg_value : %s' %p[3], (word_x, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1)

            cv2.putText(frame, 'Classification : %s' %cate[classes], (word_x, word_y), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,0,10), 2)
            out.write(frame)
...
visul_file = open('dataset/dribbling/test_videofolder.txt', 'r')
visul_list = []
for line in visul_file.readlines():
    split_item = line.split(" ", -1)
    visul_list.append(split_item[0])
visul_file.close
print(visul_list)
...
for this_weights, this_test_segments, test_file in zip(weights_list, test_segments_list, test_file_list):
    ...
    checkpoint = torch.load('pretrained/ckpt.pth.tar')
...
# 會印出所有 video 的 label id
print(video_pred)
print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
print(video_labels)

for i in range(len(visul_list)):
    path = "dataset/dribbling/dribbing-testvideo/" + visul_list[i] + ".mp4"
    output_video(path, visul_list[i], video_prob[i],  video_pred[i])
